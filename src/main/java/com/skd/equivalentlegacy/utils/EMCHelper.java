...
public final class EMCHelper {
	...
	public static long getEmcSellValue(@Range(from = 0, to = Long.MAX_VALUE) long originalValue) {
		...
		long emc = Mth.lfloor(originalValue * EquivalentLegacyConfig.server.difficulty.covalenceLoss.get());
		if (emc < 1) {
			if (EquivalentLegacyConfig.server.difficulty.covalenceLossRounding.get()) {
				emc = 1;
			} else {
				emc = 0;
			}
	...
	public static Component getEmcTextComponent(long emc, int stackSize) {
		if (EquivalentLegacyConfig.server.difficulty.covalenceLoss.get() == 1.0) {
			ILangEntry prefix;
			String value;
			if (stackSize > 1) {
