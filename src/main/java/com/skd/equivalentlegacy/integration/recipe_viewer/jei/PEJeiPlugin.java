...
import com.skd.equivalentlegacy.integration.recipe_viewer.alias.EquivalentLegacyAliasMapping;
...
public class PEJeiPlugin implements IModPlugin {
	...
	private static final ResourceLocation UID = ELCore.rl("main");
	...
	public void registerRecipeCatalysts(@NotNull IRecipeCatalystRegistration registry) {
		if (shouldLoad()) {
			...
			registry.addRecipeCatalyst(new ItemStack(PEBlocks.DARK_MATTER_FURNACE), RecipeTypes.SMELTING);
			registry.addRecipeCatalyst(new ItemStack(PEBlocks.RED_MATTER_FURNACE), RecipeTypes.SMELTING);
		}
	}
	...
	public void registerGuiHandlers(@NotNull IGuiHandlerRegistration registry) {
		if (shouldLoad()) {
			registry.addRecipeClickArea(GUIDMFurnace.class, 73, 34, 25, 16, RecipeTypes.SMELTING);
			registry.addRecipeClickArea(GUIRMFurnace.class, 88, 35, 25, 17, RecipeTypes.SMELTING);
	...
	public void registerIngredientAliases(@NotNull IIngredientAliasRegistration registration) {
		new EquivalentLegacyAliasMapping().addAliases(new JEIAliasHelper(registration));
	}
