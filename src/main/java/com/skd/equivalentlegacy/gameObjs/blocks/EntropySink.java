...
public class EntropySink extends BlockDirection implements PEEntityBlock<EntropySinkBlockEntity> {
	...
	public void addTooltip(ItemStack stack, List<Component> tooltip) {
		if (EquivalentLegacyConfig.client.statToolTips.get()) {
			tooltip.add(PELang.EMC_MAX_STORAGE.translateColored(ChatFormatting.DARK_PURPLE, ChatFormatting.BLUE,
	...
	// which is where 26.2 relied on EntropySinkBlockEntity#preRemoveSideEffects (a hook 1.21.1 does not have).
	@Override
	@Deprecated
	public void onRemove(@NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos, @NotNull BlockState newState, boolean isMoving) {
		if (!state.is(newState.getBlock()) && level.getBlockEntity(pos) instanceof EntropySinkBlockEntity sink) {
			sink.dropContentsOnRemoval(level, pos);
		}
