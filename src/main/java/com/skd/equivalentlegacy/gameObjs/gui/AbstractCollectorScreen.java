...
public abstract class AbstractCollectorScreen<T extends CollectorMK1Container> extends PEContainerScreen<T> {
	...
	protected void peExtractLabels(@NotNull GuiGraphics graphics, int mouseX, int mouseY) {
		graphics.drawString(font, Long.toString(menu.emc.get()), 60 + getBonusXShift(), 32, PEGuiGraphics.LABEL_COLOR, false);
		long kleinCharge = menu.kleinEmc.get();
		if (kleinCharge > 0) {
			graphics.drawString(font, EMCHelper.formatEmc(kleinCharge), 60 + getBonusXShift(), 44, PEGuiGraphics.LABEL_COLOR, false);
		}
	}
