package com.skd.equivalentlegacy.network.commands.client;
...
import com.skd.equivalentlegacy.emc.mappers.OreBlacklistMapper;
import com.skd.equivalentlegacy.emc.mappers.RawMaterialsBlacklistMapper;
...
public class DumpMissingEmc {
	...
	private static final boolean SKIP_TOP = Boolean.parseBoolean(System.getProperties().getProperty("equivalent_legacy.skip_top"));
	...
	private static boolean expectedMissing(FeatureFlagSet enabledFeatures, ItemInfo info) {
		...
		if (switch (holder.value()) {
			...
			case Tome tome when !EquivalentLegacyConfig.common.craftableTome.get() -> true;
			default -> false;
		}) {
			return true;
